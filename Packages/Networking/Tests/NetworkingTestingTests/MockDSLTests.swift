...
struct MockDSLTests {
  ...
  func expectationDefaults() {
    ...
    #expect(expectation.headerValues.isEmpty == true)
  ...
  func responseDefaults() {
    ...
    #expect(response.headers.isEmpty == true)
    #expect(response.error == nil)
    #expect(response.isJSON.rawValue == false)
  }
  ...
  func methodComponent() {
    ...
    if case .method(let method) = component {
      #expect(method == .get)
    } else {
  ...
  func pathComponent() {
    ...
    if case .path(let path) = component {
      #expect(path == "/users")
    } else {
  ...
  func headerPresentComponent() {
    ...
    if case .headerPresent(let name) = component {
  ...
  func statusComponent() {
    ...
    if case .status(let code) = component {
      #expect(code == 200)
    } else {
  ...
  func bodyComponent() {
    ...
    let component = Body(HTTPBody(data))
    if case .body(let body) = component {
      #expect(body.rawValue == data)
    } else {
  ...
  func mockJSONBodyComponent() throws {
    ...
    if case .jsonBody(let data) = component {
      let decoded = try JSONDecoder().decode(User.self, from: data.rawValue)
      #expect(decoded.id == 1)
  ...
  func respondBuilder() {
    let response = Respond {
      Status(.ok)
      Body(HTTPBody(Data("test".utf8)))
      ResponseHeader("X-Custom", "value")
    }

    #expect(response.statusCode == 200)
    #expect(response.body?.rawValue == Data("test".utf8))
  ...
  func networkingMockGET() async throws {
    ...
    let decoded = try JSONDecoder().decode(User.self, from: response.body!.rawValue)
  ...
  func networkingMockPOST() async throws {
    let mock = try NetworkingMock {
      ...
      Respond {
        Status(.created)
        Body(HTTPBody(Data(#"{"id": 999}"#.utf8)))
      }
    }
