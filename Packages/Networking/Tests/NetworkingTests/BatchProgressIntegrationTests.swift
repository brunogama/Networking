...
struct BatchProgressIntegrationTests {
  ...
  func batch_executesRequestsInParallel() async throws {
    ...
    // Stub all requests
    for index in 1...10 {
      client.stubGET(path: "/items/\(index)", response: #"{"id": \#(index)}"#.data(using: .utf8)!)
    }
    ...
    let indices = results.map(\.index).sorted()
  ...
  func progressTracking_streamsDownloadProgress() async throws {
    ...
    try await streamManager.updateProgress(
      for: transferId, transferredBytes: 250, phase: .downloading)
    try await streamManager.updateProgress(
      for: transferId, transferredBytes: 500, phase: .downloading)
    try await streamManager.updateProgress(
      for: transferId, transferredBytes: 1000, phase: .downloading)
  ...
  func batch_withProgressTracking_aggregatesProgress() async throws {
    ...
    for (index, _) in requests.enumerated() {
