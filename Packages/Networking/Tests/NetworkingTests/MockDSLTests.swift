...
struct MockDSLTests {
  ...
  func testHeaderPresentComponent() {
  ...
  func testMockJSONBodyComponent() throws {
    struct User: Codable {
      let id: Int
      let name: String
    }
  ...
  func testNetworkingMockGET() async throws {
  ...
  func testNetworkingMockPOST() async throws {
    let mock = try NetworkingMock {
      Expect {
        Method(.post)
        Path("/items")
