...
final class MacroExpansionTests: XCTestCase {
  ...
  func testComplexAPIWithMultipleParameters() {
    assertMacro {
      ...
      protocol ProductAPI {
          @PUT("/products/{id}")
          func updateProduct(@Path id: String, @Body product: Product, @Query force: Bool, @Header authorization: String) async throws -> Product
      }
      """
    } expansion: {
      """
  ...
  func testParameterMacrosAreInformational() {
    ...
    assertMacro {
      """
      func testMethod(@Header auth: String) {}
      """
    } expansion: {
      """
      func testMethod(@Header auth: String) {}
