...
struct ObservabilityTests {
    ...
    func requestStartedEvent() async throws {
        ...
        if case let .requestStarted(context) = events.first {
    ...
    func successfulRequestCompletion() async throws {
        ...
        if case let .requestCompleted(context, responseContext) = completionEvent {
            #expect(context.method == "GET")
    ...
    func failedRequestHandling() async throws {
        ...
        if case let .requestFailed(context, errorContext) = failureEvent {
    ...
    func customEventRecording() async throws {
        ...
        if case let .circuitBreakerTripped(endpoint, reason) = events.first {
    ...
    func retryTracking() async throws {
        ...
        let retryEvents = events.compactMap { event in
            if case let .requestRetried(_, attempt) = event {
                return attempt
            }
            return nil
        }
    ...
    func cacheEventTracking() async throws {
        ...
        let cacheEvents = events.compactMap { event -> (Bool, String)? in
            switch event {
            case let .cacheHit(_, key):
                return (true, key)

            case let .cacheMiss(_, key):
                return (false, key)
