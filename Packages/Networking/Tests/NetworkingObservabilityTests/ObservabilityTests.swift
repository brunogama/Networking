...
struct ObservabilityTests {
  ...
  func observabilityMiddlewareCreation() async throws {
  ...
  func requestStartedEvent() async throws {
    let collector = SimpleMetricsCollector()
    let middleware = NetworkObservabilityMiddleware.standard(metricsCollector: collector)
    ...
    if case .requestStarted(let context) = events.first {
      #expect(context.method.rawValue == "GET")
      #expect(context.path.rawValue == "/test-endpoint")
      #expect(context.userId?.rawValue == "user123")
      #expect(context.sessionId?.rawValue == "session456")
    } else {
  ...
  func successfulRequestCompletion() async throws {
    let collector = SimpleMetricsCollector()
    let middleware = NetworkObservabilityMiddleware.standard(metricsCollector: collector)
    ...
    if case .requestCompleted(let context, let responseContext) = completionEvent {
      #expect(context.method.rawValue == "GET")
      #expect(context.path.rawValue == "/api/users")
      #expect(responseContext.statusCode == 200)
      #expect(responseContext.statusCategory.rawValue == "success")
  ...
  func failedRequestHandling() async throws {
    let collector = SimpleMetricsCollector()
    let middleware = NetworkObservabilityMiddleware.standard(metricsCollector: collector)
    ...
    if case .requestFailed(let context, let errorContext) = failureEvent {
      #expect(context.method.rawValue == "POST")
      #expect(context.path.rawValue == "/api/upload")
      #expect(errorContext.errorType.rawValue == "network")
      #expect(errorContext.errorCategory.rawValue == "connectivity")
  ...
  func performanceMetricsCalculation() async throws {
    let collector = SimpleMetricsCollector()
  ...
  func networkHealthCalculation() async throws {
    let collector = SimpleMetricsCollector()
    let middleware = NetworkObservabilityMiddleware.standard(metricsCollector: collector)
  ...
  func customEventRecording() async throws {
    let collector = SimpleMetricsCollector()
    let middleware = NetworkObservabilityMiddleware.standard(metricsCollector: collector)

    let requestContext = try NetworkObservabilityMiddleware.RequestContext(
      from: HTTPRequest { GET("/test") }
    )
    ...
    if case .circuitBreakerTripped(let endpoint, let reason) = events.first {
      #expect(endpoint.rawValue == "/api/external")
      #expect(reason.rawValue == "High error rate detected")
    } else {
  ...
  func retryTracking() async throws {
    let collector = SimpleMetricsCollector()
    let middleware = NetworkObservabilityMiddleware.standard(metricsCollector: collector)
    ...
    let response = HTTPResponse(
      request: request,
      status: .ok,
      body: Data("Success after retries".utf8)
    )
    _ = try await middleware.processResponse(response, for: request)
  ...
  func cacheEventTracking() async throws {
    let collector = SimpleMetricsCollector()
    let middleware = NetworkObservabilityMiddleware.standard(metricsCollector: collector)
    ...
    let cacheEvents = events.compactMap { event -> (Bool, String)? in
      ...
      case .cacheHit(_, let key):
        return (true, key.rawValue)

      case .cacheMiss(_, let key):
        return (false, key.rawValue)
  ...
  func dataClearing() async throws {
    let collector = SimpleMetricsCollector()
    let middleware = NetworkObservabilityMiddleware.standard(metricsCollector: collector)
  ...
  func simpleMetricsCollector() async throws {
  ...
  func simpleMetricsCollectorLimits() async throws {
    let collector = SimpleMetricsCollector(maxEvents: 3)

    // Add more events than the limit
    for i in 0..<5 {
      let context = try NetworkObservabilityMiddleware.RequestContext(
        from: HTTPRequest { GET("/test/\(i)") }
      )
  ...
  // swiftlint:disable:next function_body_length
  func comprehensiveMetricsCollector() async throws {
  ...
  func alertGeneration() async throws {
    ...
    let errorRateAlerts = alerts.filter { $0.metric?.rawValue == "error_rate" }
    let responseTimeAlerts = alerts.filter { $0.metric?.rawValue == "response_time" }
  ...
  func observabilityCompositeMetricsCollector() async throws {
  ...
  // swiftlint:disable:next function_body_length
  func consoleMetricsCollectorFormatting() async throws {
    let collector = ConsoleMetricsCollector()

    // Test various event types
    let requestContext = try NetworkObservabilityMiddleware.RequestContext(
      from: HTTPRequest { POST("/api/submit") }
    )
  ...
  func fullObservabilityIntegration() async throws {
    ...
    let client = try NetworkClient {
      try BaseURL("https://api.example.com")
      Middleware(observabilityMiddleware as any HTTPRequestMiddleware)
    }
  ...
  func highConcurrencyObservability() async throws {
    let collector = ComprehensiveMetricsCollector.development()
  ...
  func observabilityPerformance() async throws {
  ...
  func observabilityFactoryMethods() async throws {
    let collector = SimpleMetricsCollector()
  ...
  func metricsCollectorFactoryMethods() async throws {
...
struct NetworkHealthTests {
  ...
  func networkHealthEdgeCases() async throws {
    let collector = SimpleMetricsCollector()
    let middleware = NetworkObservabilityMiddleware.standard(metricsCollector: collector)
  ...
  func performancePercentilesCalculation() async throws {
    let collector = SimpleMetricsCollector()
    let middleware = NetworkObservabilityMiddleware.standard(metricsCollector: collector)
  ...
  func businessMetricsTracking() async throws {
    let collector = ComprehensiveMetricsCollector.development()
    let middleware = NetworkObservabilityMiddleware.development(metricsCollector: collector)
...
struct ObservabilityEdgeCaseTests {
  ...
  func malformedRequestHandling() async throws {
    let collector = SimpleMetricsCollector()
    let middleware = NetworkObservabilityMiddleware.standard(metricsCollector: collector)
  ...
  func threadSafetyUnderStress() async throws {
    let collector = ComprehensiveMetricsCollector.development()
    let middleware = NetworkObservabilityMiddleware.development(metricsCollector: collector)
