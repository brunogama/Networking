...
final class SpecializedResponseTests: XCTestCase {
    ...
    func testCachedResponseBasicProperties() {
        ...
        XCTAssertTrue(cachedResponse.isCacheHit)
    ...
    func testCachedResponseFreshnessCalculation() {
        ...
        // Then
        XCTAssertTrue(cachedResponse.isFresh, "Cache should be fresh within TTL")
    ...
    func testCachedResponseExpiredCache() {
        ...
        // Then
        XCTAssertFalse(cachedResponse.isFresh, "Cache should be stale after TTL")
    ...
    func testCachedResponseNoTTL() {
        ...
        // Then
        XCTAssertTrue(cachedResponse.isFresh, "Cache without TTL should always be fresh")
    }
    ...
    func testDecodableResponseBasicDecoding() throws {
        ...
        XCTAssertEqual(decodableResponse.decodingMetadata.originalDataSize, testUserData.count)
    ...
    func testDecodableResponseInvalidJSON() {
        ...
        // When & Then
        XCTAssertThrowsError(try DecodableResponse<TestUser>.decode(from: invalidResponse)) { error in
            guard let httpError = error as? HTTPError else {
                XCTFail("Expected HTTPError")
                return
            }
            ...
            switch httpError.category {
            case let .decoding(message):
                XCTAssertTrue(message.contains("Failed to decode"), "Error should mention decoding failure")
    ...
    func testCachedDecodableResponseCreation() throws {
        ...
        XCTAssertTrue(cachedDecodableResponse.isCacheHit)
        XCTAssertTrue(cachedDecodableResponse.isFresh)
    ...
    func testCachedDecodableResponseDirectInitializer() {
        ...
        let decodingMetadata = DecodableResponse<TestUser>.DecodingMetadata(
            decoder: "JSONDecoder",
            decodedAt: Date(),
            originalDataSize: testUserData.count,
        ...
        // Then
        XCTAssertEqual(cachedDecodableResponse.value, testUser)
        XCTAssertFalse(cachedDecodableResponse.isCacheHit)
        XCTAssertEqual(cachedDecodableResponse.decodingMetadata.originalDataSize, testUserData.count)
    }
