  ...
  func testAssociativity() {
    property("Associativity: (a.appending(b)).appending(c) == a.appending(b.appending(c))")
      <- forAll(
        ...
        InterceptorChainGen.arbitrary,
        InterceptorChainGen.arbitrary
      ) { (a: InterceptorChain, b: InterceptorChain, chainC: InterceptorChain) in
        let leftAssoc = (a.appending(b)).appending(chainC)
        let rightAssoc = a.appending(b.appending(chainC))
    ...
    property("Appending preserves total interceptor counts")
      <- forAll(InterceptorChainGen.arbitrary, InterceptorChainGen.arbitrary) {
        ...
        let expectedRequestCount =
          a.requestInterceptorCount.rawValue + b.requestInterceptorCount.rawValue
        let expectedResponseCount =
          a.responseInterceptorCount.rawValue + b.responseInterceptorCount.rawValue

        return result.requestInterceptorCount.rawValue == expectedRequestCount
          && result.responseInterceptorCount.rawValue == expectedResponseCount
      }
  }

  func testEmptyChainHasZeroCounts() {
    XCTAssertEqual(InterceptorChain.empty.requestInterceptorCount.rawValue, 0)
    XCTAssertEqual(InterceptorChain.empty.responseInterceptorCount.rawValue, 0)
    XCTAssertTrue(InterceptorChain.empty.isEmpty.rawValue)
  }
  ...
  func testAppendingClosure() {
    property("Appending two chains produces a valid chain (closure)")
      <- forAll(InterceptorChainGen.arbitrary, InterceptorChainGen.arbitrary) {
        ...
        return result.requestInterceptorCount.rawValue >= 0
          && result.responseInterceptorCount.rawValue >= 0
      }
  }
