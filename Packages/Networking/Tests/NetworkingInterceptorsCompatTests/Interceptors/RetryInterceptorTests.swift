...
final class RetryInterceptorTests: XCTestCase {  // swiftlint:disable:this type_body_length
    ...
    for status in serverErrors {
      ...
      if case .retry(let delay) = result {
        ...
        XCTAssertGreaterThan(
          delay?.rawValue ?? 0,
          0,
          "Should retry with delay for status \(status.rawValue)"
        )
      } else {
  ...
  func testRetriesRequestTimeout() async throws {
    ...
    if case .retry(let delay) = result {
      ...
      XCTAssertGreaterThan(delay?.rawValue ?? 0, 0)
    } else {
  ...
  func testMaxDelayCapIsEnforced() async throws {
    ...
    if case .retry(let delay) = result {
      ...
      XCTAssertLessThanOrEqual(
        unwrappedDelay.rawValue,
        5.5,
        "Delay should be capped at maxDelay + jitter"
      )
      XCTAssertGreaterThanOrEqual(
        unwrappedDelay.rawValue,
        5.0,
        "Delay should be at least maxDelay"
      )
    } else {
      XCTFail("Expected .retry, got \(result)")
    }
  ...
  func testJitterPreventsThunderingHerd() async throws {
    ...
    for _ in 0..<10 {
      ...
      if case .retry(let delay) = result, let unwrappedDelay = delay {
        delays.append(unwrappedDelay.rawValue)
      }
    }
