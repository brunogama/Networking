...
final class ResponseInterceptorTests: XCTestCase {
    ...
    struct StatusCodeInspector: ResponseInterceptor {
        ...
        func intercept(
            ...
            onStatusCode(response.status.rawValue)
            return .proceed
        }
    }
    ...
    func testInterceptorCanInspectResponseBody() async throws {
        ...
        struct BodyInspector: ResponseInterceptor {
            ...
            func intercept(
                ...
                onBody(String(data: response.body ?? Data(), encoding: .utf8))
                return .proceed
            }
        }
    ...
    func testRetryOn401() async throws {
        ...
        if case let .retry(delay) = result {
    ...
    func testRetryWithExponentialBackoff() async throws {
        ...
        if case let .retry(delay0) = result0 {
            XCTAssertEqual(delay0, 1.0) // 2^0 = 1
        } else {
        ...
        if case let .retry(delay1) = result1 {
            XCTAssertEqual(delay1, 2.0) // 2^1 = 2
        } else {
        ...
        if case let .retry(delay2) = result2 {
            XCTAssertEqual(delay2, 4.0) // 2^2 = 4
        } else {
    ...
    func testCacheSavingOn200() async throws {
        ...
        XCTAssertEqual(savedResponse?.status.rawValue, 200)
        XCTAssertEqual(String(data: savedResponse?.body ?? Data(), encoding: .utf8), "data to cache")
    }
    ...
    func testResponseReplacement() async throws {
        ...
        if case let .shortCircuit(response) = result {
            ...
            XCTAssertEqual(String(data: response.body ?? Data(), encoding: .utf8), "replacement data")
        } else {
    ...
    func testInterceptorCanAccessMetadata() async throws {
        ...
        XCTAssertEqual(receivedMetadata?["requestId"]?.stringValue, "abc123")
        XCTAssertEqual(receivedMetadata?["priority"]?.intValue, 5)
    }
