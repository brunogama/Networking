...
final class RequestInterceptorTests: XCTestCase {  // swiftlint:disable:this type_body_length
  ...
  struct HeaderAddingInterceptor: RequestInterceptor {
    ...
    func intercept(
      ...
      headers[HTTPHeaderName(headerName)] = HTTPHeaderValue(headerValue)
      request = HTTPRequest(
  ...
  struct PathFilteringInterceptor: RequestInterceptor {
    let allowedPath: RequestPathPattern
    var callCount = 0
  ...
  func testCacheHitShortCircuits() async throws {
    ...
    if case .shortCircuit(let response) = result {
      XCTAssertEqual(
        String(
          data: (response.body ?? HTTPBody(Data())).rawValue,
          encoding: .utf8
        ),
        "cached data"
      )
  ...
  func testInterceptorCanAccessMetadata() async throws {
    ...
    XCTAssertEqual(
      receivedMetadata?[InterceptorMetadataKey("userId")]?.stringValue?.rawValue,
      "123"
    )
    XCTAssertEqual(
      receivedMetadata?[InterceptorMetadataKey("isRetry")]?.boolValue?.rawValue,
      true
    )
  }
