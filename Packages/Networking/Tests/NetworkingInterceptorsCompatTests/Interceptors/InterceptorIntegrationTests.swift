...
final class InterceptorIntegrationTests: XCTestCase {  // swiftlint:disable:this type_body_length
  // MARK: - Test Helpers
  ...
  func testCachingWithRateLimiting() async throws {
    ...
    let cache = CachingInterceptor(ttl: CacheMaxAge(300), maxEntries: CacheEntryLimit(10))
    ...
    // Verify cached
    let cached = await cache.getCachedResponse(
      for: HTTPMethod.get,
      path: RequestPathPattern("/api/users")
    )
    ...
    if case .retry = rateLimitResult3 {
      ...
      let cachedData = await cache.getCachedResponse(
        for: HTTPMethod.get,
        path: RequestPathPattern("/api/users")
      )
  ...
  func testFullStackIntegration() async throws {
    ...
    let cache = CachingInterceptor(ttl: CacheMaxAge(300))
    ...
    if case .proceed = responseResult {
      ...
      let cached = await cache.getCachedResponse(
        for: HTTPMethod.get,
        path: RequestPathPattern("/api/data")
      )
      XCTAssertNotNil(cached)
      XCTAssertEqual(
        cached?.body?.rawValue,
        Data("success data".utf8)
      )
    } else {
  ...
  func testErrorPropagationThroughChain() async throws {
    ...
    do {
      ...
      if case .rateLimitExceeded(let path, let limit, let window) = error {
        ...
        XCTAssertEqual(limit, RequestCount(1))
  ...
  func testCacheInvalidationOnRetry() async throws {
    ...
    // Verify cached
    let cached1 = await cache.getCachedResponse(for: HTTPMethod.get, path: "/api/data")
    ...
    if case .retry = retryResult {
      ...
      let cached2 = await cache.getCachedResponse(for: HTTPMethod.get, path: "/api/data")
