...
final class TokenRefreshInterceptorTests: XCTestCase {
    ...
    func testRefreshesTokenOn401Response() async throws {
        ...
        let interceptor = TokenRefreshInterceptor(
            refreshHandler: { refreshToken in
                ...
                XCTAssertEqual(refreshToken, "old-refresh-token")
        ...
        if case let .retry(delay) = result {
            XCTAssertEqual(delay, 0, "Should retry immediately")
        } else {
    ...
    func testCallsTokenUpdateHandler() async throws {
        ...
        let interceptor = TokenRefreshInterceptor(
            ...
            tokenUpdateHandler: { accessToken, refreshToken in
                await storage.update(accessToken: accessToken, refreshToken: refreshToken)
            }
        )
