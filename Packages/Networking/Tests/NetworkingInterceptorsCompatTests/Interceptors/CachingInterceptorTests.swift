...
final class CachingInterceptorTests: XCTestCase {  // swiftlint:disable:this type_body_length
  // MARK: - Basic Caching
  ...
  func testCachesSuccessfulGETResponse() async throws {
    ...
    XCTAssertEqual(cached?.body?.rawValue, responseData)
  }
  ...
  func testWorksWithInterceptorChain() async throws {
    ...
    XCTAssertNotNil(cached)
    XCTAssertEqual(cached?.body?.rawValue, Data("chain data".utf8))
  }
}
