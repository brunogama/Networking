...
final class ErrorHandlingTests: XCTestCase {
  ...
  func testErrorClientServerClassification() {
    ...
    XCTAssertTrue(clientError.isClientError.rawValue)
    XCTAssertFalse(clientError.isServerError.rawValue)

    let serverError = HTTPError(category: .http(.internalServerError), request: testRequest)
    XCTAssertFalse(serverError.isClientError.rawValue)
    XCTAssertTrue(serverError.isServerError.rawValue)

    let networkError = HTTPError(category: .network(.serverUnreachable), request: testRequest)
    XCTAssertFalse(networkError.isClientError.rawValue)
    XCTAssertTrue(networkError.isServerError.rawValue)
  }

  func testTransientErrorDetection() {
    let transientError = HTTPError(category: .network(.connectionLost), request: testRequest)
    XCTAssertTrue(transientError.isTransientError.rawValue)

    let nonTransientError = HTTPError(category: .decoding("Invalid JSON"), request: testRequest)
    XCTAssertFalse(nonTransientError.isTransientError.rawValue)
  }
  ...
  func testAutoExecutableActions() {
    ...
    XCTAssertTrue(autoExecutable.allSatisfy { $0.canAutoExecute.rawValue })
  }
  ...
  func testAutomaticRetryStrategy() async {
    ...
    let transientError = HTTPError(category: .network(.connectionLost), request: testRequest)
    XCTAssertTrue(strategy.canRecover(from: transientError).rawValue)

    let nonTransientError = HTTPError(
      category: .http(.notFound),
      request: testRequest
    )
    XCTAssertFalse(strategy.canRecover(from: nonTransientError).rawValue)
  ...
  func testAuthenticationRefreshStrategy() async {
    ...
    let authError = HTTPError(category: .http(.unauthorized), request: testRequest)
    XCTAssertTrue(strategy.canRecover(from: authError).rawValue)

    let notAuthError = HTTPError(category: .http(.notFound), request: testRequest)
    XCTAssertFalse(strategy.canRecover(from: notAuthError).rawValue)
  ...
  func testCircuitBreakerStrategy() async {
    ...
    let serverError = HTTPError(category: .http(.internalServerError), request: testRequest)
    XCTAssertTrue(strategy.canRecover(from: serverError).rawValue)
  ...
  func testCompositeRecoveryStrategy() async {
    ...
    let authError = HTTPError(category: .http(.unauthorized), request: testRequest)
    XCTAssertTrue(composite.canRecover(from: authError).rawValue)

    let networkError = HTTPError(category: .network(.connectionLost), request: testRequest)
    XCTAssertTrue(composite.canRecover(from: networkError).rawValue)
  }
  ...
  func testCustomRecoveryStrategy() async {
    let customStrategy = ErrorRecoveryStrategies.CustomRecoveryStrategy(
      ...
      canRecover: { error in
        RetryDecision(error.category == .timeout)
      },
    ...
    let timeoutError = HTTPError(category: .timeout, request: testRequest)
    XCTAssertTrue(customStrategy.canRecover(from: timeoutError).rawValue)

    let networkError = HTTPError(category: .network(.noConnection), request: testRequest)
    XCTAssertFalse(customStrategy.canRecover(from: networkError).rawValue)
