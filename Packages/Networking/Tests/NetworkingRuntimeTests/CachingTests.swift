...
struct CachingTests {
  ...
  func cacheEntryBasics() async throws {
    ...
    #expect(entry.isExpired == false)  // Should not be expired immediately
    #expect(entry.age >= 0)
  ...
  func cacheEntryExpiration() async throws {
    ...
    #expect(entry.isExpired.rawValue)
  ...
  func memoryCacheBasicOperations() async throws {
    let cache = MemoryCacheStorage(maxSize: 10)
    let key: CacheKey = "test-key"
    let response = createTestResponse()
    let entry = CachingMiddleware.CacheEntry(
      response: response,
      ttl: 300.0
  ...
  func memoryCacheSizeLimits() async throws {
    let cache = MemoryCacheStorage(maxSize: 2)

    // Add entries beyond limit
    for i in 0..<5 {
      let key: CacheKey = "key-\(i)"
      let response = createTestResponse()
      let entry = CachingMiddleware.CacheEntry(
        response: response,
        ttl: 300.0
  ...
  func memoryCacheExpiredCleanup() async throws {
    let cache = MemoryCacheStorage()
    let key: CacheKey = "expired-key"
    let response = createTestResponse()
    let entry = CachingMiddleware.CacheEntry(
      response: response,
  ...
  func advancedMemoryCacheLRU() async throws {
  ...
  func advancedMemoryCacheTagInvalidation() async throws {
    let cache = AdvancedMemoryCacheStorage()
    let response = createTestResponse()
  ...
  func advancedMemoryCacheMetrics() async throws {
    let cache = AdvancedMemoryCacheStorage()
    let response = createTestResponse()
    let entry = CachingMiddleware.CacheEntry(response: response, ttl: 300.0)
    ...
    #expect(metrics.hitRatio.rawValue == 0.5)
  ...
  func cachingMiddlewareBasic() async throws {
  ...
  func cachingMiddlewareWithCacheControl() async throws {
    let client = createTestClient()
    let storage = MemoryCacheStorage()
    let configuration = CachingMiddleware.Configuration()
  ...
  func cachingMiddleware304Handling() async throws {
    let client = createTestClient()
    let storage = MemoryCacheStorage()
    let configuration = CachingMiddleware.Configuration()
    ...
    let bodyString = String(data: finalResponse.body?.rawValue ?? Data(), encoding: .utf8)
  ...
  func cacheMetadataAndIntelligentKeys() async throws {
    ...
    #expect(generatedKey.isEmpty == false)
  }

  @Test("Cache invalidation with metadata")
  func cacheInvalidationWithMetadata() async throws {
  ...
  func cachePerformanceConcurrentAccess() async throws {
    let cache = AdvancedMemoryCacheStorage(
      policy: .lru,
    ...
    await withTaskGroup(of: Void.self) { group in
      for i in 0..<100 {
        group.addTask {
          let key: CacheKey = "concurrent-key-\(i)"
          await cache.set(key, entry: entry)
        }
      }
    }
    ...
    await withTaskGroup(of: Bool.self) { group in
      ...
      for await hit in group where hit {
        hitCount += 1
      }
    }
  ...
  func memoryCacheEvictionPerformance() async throws {
    let cache = AdvancedMemoryCacheStorage(
      policy: .lru,
  ...
  func cacheKeyNormalizationAndCollisions() async throws {
    let cache = MemoryCacheStorage()
    let response = createTestResponse()
    let entry = CachingMiddleware.CacheEntry(response: response, ttl: 300.0)
  ...
  func endToEndCachingWorkflow() async throws {
    let client = createTestClient()
    let storage = AdvancedMemoryCacheStorage(
      policy: .lru,
      sizePolicy: .maxEntries(100)
    ...
    let cachedBodyString = String(
      data: processedResponse2.body?.rawValue ?? Data(),
      encoding: .utf8
    )
    ...
    #expect(metrics.hitCount >= 1)
    #expect(metrics.hitRatio.rawValue > 0.0)
  }

  @Test("Cache behavior with different HTTP methods")
  func cacheWithDifferentHTTPMethods() async throws {
    let client = createTestClient()
    let storage = MemoryCacheStorage()
    let configuration = CachingMiddleware.Configuration()
