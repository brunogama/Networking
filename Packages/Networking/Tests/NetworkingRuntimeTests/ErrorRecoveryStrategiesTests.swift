...
final class ErrorRecoveryStrategiesTests: XCTestCase {
  ...
  func testCanRecoverFromTransientNetworkErrors() {
    ...
    for error in transientErrors {
      XCTAssertTrue(
        strategy.canRecover(from: error).rawValue,
  ...
  func testCanRecoverFrom503ServiceUnavailable() {
    ...
    XCTAssertTrue(strategy.canRecover(from: error).rawValue)
  }
  ...
  func testCannotRecoverFrom504GatewayTimeout() {
    ...
    XCTAssertFalse(strategy.canRecover(from: error).rawValue)
  }
  ...
  func testCompositeRecoveryStrategyCanRecoverCombined() {
    ...
    XCTAssertTrue(composite.canRecover(from: timeoutError).rawValue)
  ...
  func testCompositeRecoveryStrategyEmptyStrategies() {
    ...
    XCTAssertFalse(composite.canRecover(from: error).rawValue)
  }
  ...
  func testCustomRecoveryStrategyMaxAttempts() {
    let strategy = ErrorRecoveryStrategies.CustomRecoveryStrategy(
      ...
      canRecover: { _ in RetryDecision(true) },
      recovery: { _, _, _ in
  ...
  func testCustomRecoveryStrategyCanRecoverPredicate() {
    let strategy = ErrorRecoveryStrategies.CustomRecoveryStrategy(
      canRecover: { error in
        if case .http(let status) = error.category {
          return RetryDecision(status.rawValue == 429)
        }
    ...
    XCTAssertTrue(strategy.canRecover(from: rateLimitError).rawValue)
    XCTAssertFalse(strategy.canRecover(from: otherError).rawValue)
  }
  ...
  func testComprehensiveRecoveryFactory() {
    ...
    XCTAssertTrue(strategy.canRecover(from: timeoutError).rawValue)
