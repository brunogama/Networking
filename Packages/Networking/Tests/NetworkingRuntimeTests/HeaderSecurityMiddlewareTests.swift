...
final class HeaderSecurityMiddlewareTests: XCTestCase {
  ...
  func testStrictConfigurationValues() {
    XCTAssertTrue(strictConfig.validateHeaderNames.rawValue)
    XCTAssertTrue(strictConfig.validateHeaderValues.rawValue)
    XCTAssertTrue(strictConfig.sanitizeHeaders.rawValue)
    XCTAssertEqual(strictConfig.maxHeaderValueLength, 2048)
    XCTAssertEqual(strictConfig.maxHeaderCount, 30)
    XCTAssertTrue(strictConfig.removeDangerousHeaders.rawValue)
  }

  func testPermissiveConfigurationValues() {
    XCTAssertTrue(permissiveConfig.validateHeaderNames.rawValue)
    XCTAssertTrue(permissiveConfig.validateHeaderValues.rawValue)
    XCTAssertTrue(permissiveConfig.sanitizeHeaders.rawValue)
    XCTAssertEqual(permissiveConfig.maxHeaderValueLength, 8192)
    XCTAssertEqual(permissiveConfig.maxHeaderCount, 100)
    XCTAssertFalse(permissiveConfig.removeDangerousHeaders.rawValue)
  }

  func testAPIConfigurationValues() {
    let apiConfig = HeaderSecurityMiddleware.Configuration.api
    XCTAssertTrue(apiConfig.validateHeaderNames.rawValue)
    XCTAssertTrue(apiConfig.validateHeaderValues.rawValue)
    XCTAssertFalse(apiConfig.sanitizeHeaders.rawValue)  // Fail fast for APIs
    XCTAssertEqual(apiConfig.maxHeaderValueLength, 1024)
    XCTAssertEqual(apiConfig.maxHeaderCount, 20)
    XCTAssertTrue(apiConfig.removeDangerousHeaders.rawValue)
  }
  ...
  func testValidHeaderNamesAccepted() async throws {
    ...
    XCTAssertEqual(result.headers.count.rawValue, validHeaders.count)
  }
  ...
  func testCompleteSecurityValidation() async throws {
    ...
    XCTAssertEqual(result.url.rawValue, URL(string: "https://api.example.com/data")!)
    XCTAssertEqual(result.body, HTTPBody(Data("test".utf8)))
  }
}
