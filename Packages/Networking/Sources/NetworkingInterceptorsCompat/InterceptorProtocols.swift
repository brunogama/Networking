...
/// - Important: Interceptors are kept for compatibility. Prefer `HTTPRequestMiddleware` in new
///   runtime extensions.
@available(
  *,
  deprecated,
  message:
    "Interceptors are a compatibility API. Prefer HTTPRequestMiddleware, HTTPResponseMiddleware, and HTTPErrorMiddleware for new runtime extensions."
)
...
///     context: InterceptorContext
///   ) async throws -> InterceptorResult {
///     let summary = "[\(context.method)] \(context.path) -> \(response.statusCode)"
///     _ = (summary, response.headers)
///     return .proceed
///   }
/// }
/// ```
...
/// - Important: Interceptors are kept for compatibility. Prefer `HTTPResponseMiddleware` and
///   `HTTPErrorMiddleware` in new runtime extensions.
@available(
  *,
  deprecated,
  message:
