...
public enum NetworkingConfiguration {
  ...
  public enum RetryDelays {
    ...
    public static let immediate = RetryDelay(rawValue: 0.5)

    /// Connection lost scenarios - moderate delay to allow network recovery
    public static let connectionLost = RetryDelay(rawValue: 2.0)

    /// Server unreachable - longer delay for infrastructure issues
    public static let serverUnreachable = RetryDelay(rawValue: 30.0)

    /// Rate limiting scenarios - respect API rate limits
    public static let rateLimiting = RetryDelay(rawValue: 60.0)

    /// Server errors (5xx) - moderate delay for server recovery
    public static let serverError = RetryDelay(rawValue: 15.0)

    /// Timeout scenarios - shorter delay for quick retry
    public static let timeout = RetryDelay(rawValue: 5.0)

    /// Default fallback delay for unspecified scenarios
    public static let `default` = RetryDelay(rawValue: 10.0)
  }

  /// Cache-related configuration constants
  public enum Cache {
    /// Default cache duration for successful responses
    public static let defaultTTL = CacheMaxAge(rawValue: 300)  // 5 minutes

    /// Maximum cache size in bytes
    public static let maxCacheSize = StorageSizeValue(50 * 1024 * 1024)  // 50MB

    /// Cache cleanup interval
    public static let cleanupInterval = CacheMaxAge(rawValue: 3600)  // 1 hour
  }

  /// Network timeout configuration
  public enum Timeouts {
    /// Default request timeout
    public static let request = RequestTimeout(rawValue: 30.0)

    /// Default resource timeout
    public static let resource = SessionRequestTimeout(rawValue: 60.0)

    /// Connection timeout
    public static let connection = SessionRequestTimeout(rawValue: 10.0)
  }

  /// Logging configuration
  public enum Logging {
    /// Maximum request body size to log (in bytes)
    public static let maxRequestBodyLogSize = LogBodyLengthLimit(rawValue: 1024)

    /// Maximum response body size to log (in bytes)
    public static let maxResponseBodyLogSize = LogBodyLengthLimit(rawValue: 2048)
  }
}
