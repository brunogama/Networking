...
public final class MockURLProtocol: URLProtocol, @unchecked Sendable {
  ...
  public struct RequestStub: Sendable {
    ...
    func matches(_ request: URLRequest) -> Bool {
      if let contextID, request.value(forHTTPHeaderField: MockURLProtocol.contextHeader) != contextID {
        return false
      }
  ...
  private static func request(_ request: URLRequest, matchesContextID contextID: String?) -> Bool {
    guard let contextID else { return true }
    return request.value(forHTTPHeaderField: contextHeader) == contextID
  }
  ...
  public static func stubError(url: String, error: Error, contextID: String? = nil) {
    stub(matching: .url(url), response: .failure(error), contextID: contextID)
  }

  /// Stub a URL with timeout
  /// - Parameter url: The URL to match
  public static func stubTimeout(url: String, contextID: String? = nil) {
  ...
  public static func getCapturedRequests(contextID: String? = nil) async -> [URLRequest] {
  ...
  public static func getRequestCount(for url: String, contextID: String? = nil) async -> Int {
  ...
  public static func clearAll(contextID: String? = nil) {
    let semaphore = DispatchSemaphore(value: 0)
    Task.detached(priority: .userInitiated) { @Sendable in
  ...
  public static func createMockConfiguration(contextID: String? = nil) -> URLSessionConfiguration {
    let config = URLSessionConfiguration.ephemeral
    config.protocolClasses = [MockURLProtocol.self]
    if let contextID {
      var headers = config.httpAdditionalHeaders ?? [:]
      headers[contextHeader] = contextID
  ...
  public static func createMockHTTPClient(contextID: String? = nil) -> HTTPClient {
