...
public final class MockRequestInterceptor: RequestInterceptor, MockVerifiable, @unchecked Sendable {
  ...
  private var _interceptHandler: (@Sendable (inout HTTPRequest, InterceptorContext) async throws ->
    InterceptorResult)?
  ...
  public func intercept(
    ...
    let handler = queue.sync(flags: .barrier) { () -> (@Sendable (inout HTTPRequest, InterceptorContext) async throws -> InterceptorResult)? in
      _interceptCount += 1
  ...
  public func stubIntercept(
    _ handler: @escaping @Sendable (inout HTTPRequest, InterceptorContext) async throws ->
      InterceptorResult
  ) {
