...
///
/// Generates code for PATCH requests with request body support.
///
/// Example:
/// ```swift
...
public struct PATCHMacro: PeerMacro {
  public static func expansion(
    ...
    guard let function = declaration.as(FunctionDeclSyntax.self) else {
      MacroHelpers.emitError(
        "@PATCH can only be applied to function declarations",
        node: node,
        context: context
      )
      return []
    ...
    guard let returnType = MacroHelpers.extractReturnType(from: function) else {
      MacroHelpers.emitError(
        "@PATCH methods must have an explicit return type",
        node: function,
        context: context
      )
      return []
  ...
  private static func extractPath(
    ...
    guard let arguments = node.arguments?.as(LabeledExprListSyntax.self),
      ...
      MacroHelpers.emitError(
        "@PATCH requires a path argument",
        node: node,
        context: context
      )
      return nil
  ...
  private static func extractBodyParameter(
    ...
    guard let arguments = node.arguments?.as(LabeledExprListSyntax.self) else {
      MacroHelpers.emitError(
        "@PATCH requires a 'body' argument",
        node: node,
        context: context
      )
      return nil
  ...
  /// Generates the implementation code for the PATCH request.
